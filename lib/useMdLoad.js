...
module.exports = function (config) {
  function getMdStrByUrl (url) {
    ...
    if (url === '/') {
      ...
      const match_path = m[1].split("--").map(item => urlencode.decode(item,'utf8')).join('/')
  ...
  function getMdStrByQuery (queryParams) {
    if(!queryParams) return ""
    let filepath
    if (queryParams.startsWith('mds-')) {
      const matchReg = /mds-([^\/\?]*).*/
      const m = queryParams.match(matchReg)
      const match_path = m[1].split("--").map(item => urlencode.decode(item,'utf8')).join('/')
  ...
  function getMarkdownHtml (str) {
    ...
    // markdown
    const markdown_container = `<div id="markdown_container" class="">
  ...
  ` // 加入js脚本
    ...
    const html_body = `<div id="main" class="markdown-body module">${content}</div>${scripts}`
    // 写入拼接内容
  ...
  async function mkdirByType(type) {
    if(!type) {
      return false
    }
    let mddirPath = path.resolve(process.cwd(), config.mds_dir)
    const dirs = type.split("-")
    for(let i = 0;i < dirs.length; i++) {
      mddirPath = path.resolve(mddirPath, dirs[i])
      try {
       await fs_promises.stat(mddirPath)
      } catch(err) {
        await fs_promises.mkdir(mddirPath)
      }
