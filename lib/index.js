...
module.exports = async function (config) {
  ...
  function replaceBodyContainer(html_string, str, isEnd) {
    return html_string.replace(content_dom, isEnd ? str : (str + '\r\n' + content_dom))
  }
 ...
 /* ------------------------------------      通用html拼接方法      ------------------------------------------------------ */
  function replaceHtml (html_string, html) {
    ...
    if(match_body) {
      tempstr=tempstr.replace(match_body[0], match_body[0].replace(match_body[1], `<section id="container">${match_body[1]}${content_dom}</section>`))
    } else {
      tempstr = replaceBody(tempstr, `<section id="container">${content_dom}</section>`, false)
    }
    ...
    if (showType) {
      tempstr = replaceBodyContainer(tempstr,
        `
...
${tags_module_html}
  ...
  server.on('request', function (request, response) {
    ...
    if (url_index.startsWith("/pass") && config.need_password) { // 密码验证逻辑
      ...
      response.writeHead(301, { 'Location': mdUrlIndex + `?md=${urlencode.encode(url_index.substring(1), 'utf8')}` });
      response.end()
      ...
      function getMdHtml (url) {
        ...
        mdstr = mdstr.replace(mdstr.match(/(?<=(---))\s*\n([\s\S]*)\n\s*(?=(---))/)[0], "")
        ...
        if (markdown_html && md_html) {
          ...
          html_string = replaceTitle(html_string, mdinfo.title)
          html_string = replaceMeta(html_string, `<meta name="description" content="${mdinfo.desp}"/> `)
          html_string = replaceMeta(html_string, `<meta name="keywords" content="${mdinfo.keywords}"/> `)
          html_string = replaceHeader(html_string, markdown_html.match(/<head>([\s\S]*)<\/head>/)[1])
          return replaceBodyContainer(html_string,  markdown_html.match(/<body>([\s\S]*)<\/body>/)[1], true)
        } else {
          return ""
        }
      }
      ...
      html_string = renderEjs(html_string, html, url_index, {mds,})
      if (html.title) {
      ...
      html_string =   replaceBodyContainer(html_string,  "", true)
