...
module.exports = async function (config) {
  ...
  const {  getHeaders,
    getFooter, getMdInfos, getTypeModule, getTagModule,
  ...
  const type_module = getTypeModule(mdInfos)
  const tag_module = getTagModule(mdInfos)
  ...
  function replaceHtml(html_string, html) {
    ...
    if(showType ) {
      tempstr = replaceBody(tempstr,type_module,true)
    }
    if(showTag ) {
      tempstr = replaceBody(tempstr,tag_module,true)
    }
    if(showHeader) {
    ...
    if (url_index.startsWith("/pass") && config.need_password) { // 密码验证逻辑
      ...
      if(html.content) {
        if(html.content === 'types') {
          html_string=  html_string.replace(content_dom, type_module)
        }
        if(html.content === 'tags') {
          html_string=  html_string.replace(content_dom, tag_module)
        }
      }
