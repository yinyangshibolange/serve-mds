...
module.exports = async function (config) {
    ...
    const {
        openDefaultBrowser, replaceHeader,
        replaceBody,
        replaceMeta,
        replaceTitle,
        resToJson,
    } = require("./useUtils")(config)
    const {getMdStrByUrl, getMdStrByQuery, getMarkdownHtml, mkdirByType} = require("./useMdLoad")(config)
    ...
    async function resetMdInfos() {
        mdInfos = await getMdInfos(md_path)
        const _type = getTypeModule(mdInfos)
        type_module_html = _type.type_module_html
        types = _type.types

        const _tag = getTagModule(mdInfos)
        tags_module_html = _tag.tags_module_html
        tags = _tag.tags

        const _timeline = getTimeLineModule(mdInfos)
        timeline = _timeline.timeline
        timeline_module_html = _timeline.timeline_module_html

        mds = getMdList(mdInfos)
    }
    ...
    server.on('request', function (request, response) {
        ...
        if (url_index.startsWith("/pass") && config.need_password) { // 密码验证逻辑
        ...
        } else if (url_index.startsWith("/api/admin/deleteType")) {
            if (config.need_password) {
                // 验证cookie中的密码
                if (!validate_password(request.headers.cookie)) {
                    // 失败
            ...
            fs.readdir(parentPath, async (err, files) => {
                if (err) {
                    response.writeHead(301, {'Location': `/md-edit.html?success=0&message=${urlencode.encode('删除失败，分类不存在', 'utf-8')}`});
                    response.end()
                } else {
                    if (files && files.length > 0) {
                        response.writeHead(301, {'Location': `/md-edit.html?success=0&message=${urlencode.encode('删除失败，文件夹下存在其他文件，请确认！', 'utf-8')}`});
                        response.end()
                    } else {
                        await fs_promises.rmdir(parentPath)
                        await resetMdInfos()
                        response.writeHead(301, {'Location': `/md-edit.html?success=1&message=${urlencode.encode('删除成功', 'utf-8')}`});
                        response.end()
                    }
                }
            })
            ...
            form.parse(request, async function (err, fields, files) {
                ...
                let {title, author, desp, time, type, tag, recommend, istop, oldBanner} = fields

                const content = fields['editor-markdown-doc'] ? (fields['editor-markdown-doc'][0] || "") : ''
                ...
                oldBanner = oldBanner ? (oldBanner[0] || "") : ""

                await mkdirByType(type) // 创建文件夹by type

                let banner
                if (oldBanner) {
                    banner = oldBanner
                } else {
                    if (files.banner && files.banner[0] && files.banner[0].size > 0) {
                        const _m = files.banner[0].path.match(/\\([^\\]*\.\w+)/)
                        banner = _m ? ("/uploads/banners/" + _m[1]) : ""
                    } else {
                        banner = ""
                    }
                }
...
${content}`
                ...
                let mdPath
                if (findObj) {
                    mdPath = path.resolve(parentPath, findObj.index.replace('/mds-', '').split("--").join("/") + '.md')
                } else {
                    if (type) {
                        type.split("-").forEach(item => {
                            parentPath = path.resolve(parentPath, item)
                    ...
                    mdPath = path.resolve(parentPath, title + '.md')
                }
                try {
                    await fs_promises.writeFile(mdPath, newMdStr)
                    await resetMdInfos()
                    response.writeHead(301, {'Location': `/md-edit.html?type=${urlencode.encode(type)}&title=${urlencode.encode(title)}&success=1`});
                    return response.end()
                } catch (err) {
                    response.writeHead(301, {'Location': `/md-edit.html?type=${urlencode.encode(type)}&title=${urlencode.encode(title)}&success=0&message=error`});
                    return response.end()
                }
            });


        } else if (url_index.startsWith("/api/admin/deleteMd")) {
            if (config.need_password) {
                // 验证cookie中的密码
                if (!validate_password(request.headers.cookie)) {
                    // 失败
            ...
            if (findObj) {
                const mdPath = path.resolve(parentPath, findObj.index.replace('/mds-', '').split("--").join("/") + '.md')
                fs.unlink(mdPath, async (err) => {
                    if (err) {
                        ...
                        await resetMdInfos()
                        response.writeHead(301, {'Location': `/md-edit.html?success=1&message=deleted`});
                        response.end()
                    }
                })
            } else {
                response.writeHead(301, {'Location': `/md-edit.html?success=0&message=file_not_find`});
                response.end()
            }
            ...
            response.setHeader('Cache-Control', md_html['Cache-Control']);
            ...
            markdown_html = markdown_html ? replaceBody(markdown_html, config.body_top_jss.map(item => `<script src="${item}"></script>`).join("\n"), true) : ""
            if (markdown_html) {
            ...
            html_string = html_string ? replaceBody(html_string, config.body_top_jss.map(item => `<script src="${item}"></script>`).join("\n"), true) : ""
